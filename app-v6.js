(() => {
  ...
  function positionDropIndicator(pointerY) {
  ...
  function randomiseAll() {
    wordCountInput.value = String(4 + secureInteger(3));
    numberCountInput.value = String(1 + secureInteger(8));
    specialCountInput.value = String(1 + secureInteger(4));
    separatorInput.value = separators[secureInteger(separators.length)];

    for (const item of secureShuffle(orderItems())) {
      orderList.insertBefore(item, dropIndicator);
    }

    randomOrderInput.checked = true;
    updateControlValues();
    updateOrderEditor();
    generatePassword();
  ...
  randomiseAllButton.addEventListener("click", randomiseAll);
