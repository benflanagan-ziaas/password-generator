(() => {
  ...
  const copyFlight = document.getElementById("copy-flight");
  const copyFlightPassword = document.getElementById("copy-flight-password");
  ...
  function regeneratePassword() {
    generatePassword();
    restartAnimation(passwordBox, "is-updating");
    restartAnimation(passwordOutput, "is-updating");
  }
  ...
  function animateCopyFlight(password) {
    if (matchMedia("(prefers-reduced-motion: reduce)").matches) {
      return;
    }

    copyFlightAnimation?.cancel();
    copyFlightPassword.textContent = password;

    const sourceBounds = passwordBox.getBoundingClientRect();
    const destinationBounds = copyButton.getBoundingClientRect();
    const sourceIsVisible = sourceBounds.bottom > 0 && sourceBounds.top < innerHeight;
    const endX = destinationBounds.left + (destinationBounds.width / 2);
    const endY = destinationBounds.top + (destinationBounds.height / 2);
    const startX = sourceIsVisible
      ? sourceBounds.left + (sourceBounds.width / 2)
      : innerWidth / 2;
    const startY = sourceIsVisible
      ? sourceBounds.top + (sourceBounds.height / 2)
      : Math.max(72, endY - 180);
    const arcX = startX + ((endX - startX) * 0.52);
    const arcY = Math.min(startY, endY) - Math.min(90, 36 + (Math.abs(endY - startY) * 0.18));

    copyFlightAnimation = copyFlight.animate(
      [
        {
          opacity: 0,
          transform: `translate(${startX}px, ${startY}px) translate(-50%, -50%) scale(0.82) rotate(-7deg)`
        },
        {
          offset: 0.14,
          opacity: 1,
          transform: `translate(${startX}px, ${startY}px) translate(-50%, -50%) scale(1) rotate(-4deg)`
        },
        {
          offset: 0.58,
          opacity: 1,
          transform: `translate(${arcX}px, ${arcY}px) translate(-50%, -50%) scale(0.9) rotate(5deg)`
        },
        {
          opacity: 0,
          transform: `translate(${endX}px, ${endY}px) translate(-50%, -50%) scale(0.3) rotate(18deg)`
        }
      ],
      {
        duration: 980,
    ...
    copyFlightAnimation.addEventListener("finish", () => {
      copyFlightPassword.textContent = "";
    }, { once: true });
  }
  ...
  copyButton.addEventListener("click", () => {
    ...
    animateCopyFlight(passwordOutput.textContent);
    copyPassword();
  });
  settingsToggle.addEventListener("click", () => {
    const isOpen = settingsPanel.classList.toggle("is-open");
    settingsToggle.setAttribute("aria-expanded", String(isOpen));
    status.textContent = `Settings ${isOpen ? "expanded" : "collapsed"}`;
  });
